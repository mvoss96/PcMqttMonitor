...
Both stream sinks send the same compact JSON object as the MQTT `status` topic — one object per datagram (UDP) or per line (TCP):
...
UDP is fire-and-forget (ideal for microcontrollers/ESP32 displays); TCP clients that stop reading are dropped after a short write timeout so they can never stall publishing.
